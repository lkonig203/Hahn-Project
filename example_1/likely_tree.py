...
file2 = ast.literal_eval(open('newDictionary.txt', 'r').read()) # Converts string dictionary to dictionary
file = open("trees.tre")
...
for line in file:
    ...
    try:
        ...
        currentTree = Tree(line) #file[0] is the true tree
        treeHeight = thisTree.dist
        interval_quantity = thisTree.dist #Finds top interval 
        
        #Generates alternative trees 
        cleaned_tree = removeDuplicateTrees(find_valid(generate_alt(assignValues(hide_integers([line]))), [line]))
        ...
        print("True Tree: ", line) 
        print("Predicted Tree: ",max_key)
        print("------------------------------------------------------")
        savedValues['True']+=[line]
        savedValues['Total Duplication']+=[0]
        savedValues['Predicted']+=[max_key]
        savedValues['RF']+=[isequal_value(line,max_key)]
