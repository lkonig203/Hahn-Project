...
class readWrite:
    ...
    def parse_bio_with_branch_length(self,newick):
            import random
            import re
            ...
            def recurse(tre, nextid=0, parentid=-1): 
                ...
                if ch == "(":
                    ...
                    name, length, delim, ch = next(tokens).groups(0)
                    if length:
                        tre.branchlength = max(tre.leftChild.branchlength, tre.rightChild.branchlength)
                        # tre.higher_length=float(length)+max(tre.leftChild.higher_length, tre.rightChild.higher_length)
                        # tre.lower_length= max(tre.leftChild.higher_length, tre.rightChild.higher_length)
    ...
    def to_newick1(self,tree):
        newick = ""
        newick = self.traverse1(tree, newick)
        newick = f"{newick};"
        return newick
    ...
    def write_events1(self,tree):
        if tree.isLeaf:
            return tree.numbered_taxa+':'+str(tree.key)  
        else:
            eve=':'+str(tree.key)
                
            return eve

    def traverse1(self,tree, newick):
        if tree.leftChild and not tree.rightChild:
            newick = f"(,{self.traverse1(tree.leftChild, newick)}){self.write_events1(tree)}"
        elif not tree.leftChild and tree.rightChild:
            newick = f"({self.traverse1(tree.rightChild, newick)},){self.write_events1(tree)}"
        elif tree.leftChild and tree.rightChild:
            newick = f"({self.traverse1(tree.rightChild, newick)},{self.traverse1(tree.leftChild, newick)}){self.write_events1(tree)}"
        elif not tree.leftChild and not tree.rightChild :
            newick = f"{self.write_events1(tree)}"
        else:
            pass
        return newick
    ...
    def to_newick2(self,tree):
        newick = ""
        newick = self.traverse2(tree, newick)
        newick = f"{newick};"
        return newick
    
    def traverse2(self,tree, newick):
        if tree.leftChild and not tree.rightChild:
            newick = f"(,{self.traverse(tree.leftChild, newick)}){self.write_events2(tree)}"
        elif not tree.leftChild and tree.rightChild:
            newick = f"({self.traverse(tree.rightChild, newick)},){self.write_events2(tree)}"
        elif tree.leftChild and tree.rightChild:
            newick = f"({self.traverse(tree.rightChild, newick)},{self.traverse(tree.leftChild, newick)}){self.write_events2(tree)}"
        elif not tree.leftChild and not tree.rightChild :
            newick = f"{self.write_events2(tree)}"
        else:
            pass
        return newick
    
    def write_events2(self,tree):
        if tree.isLeaf:
            return tree.numbered_taxa
        return ''
